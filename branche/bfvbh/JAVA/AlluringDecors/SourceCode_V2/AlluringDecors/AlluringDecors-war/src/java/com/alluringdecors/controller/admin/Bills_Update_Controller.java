...
@WebServlet(name = "Bills_Update_Controller", urlPatterns = {"/Bills_Update_Controller"})
public class Bills_Update_Controller extends BaseController {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            BillsDTO item = bean.selectById(Integer.parseInt(request.getParameter("ID")));
            ...
            if (paidAmount > balanceAmount) {
                item.setDueAmount(paidAmount - balanceAmount);
                item.setBalanceAmount(0.0);
            } else if (paidAmount < balanceAmount) {
                item.setDueAmount(0.0);
                item.setBalanceAmount(balanceAmount - paidAmount);
            } else {
                item.setDueAmount(0.0);
                item.setBalanceAmount(0.0);
            }
